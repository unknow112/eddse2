...
VALID_SET_FN=tmp/raw_data/doc_ids_paper_has_author.txt

bzip2 -c -d tmp/raw_data/paper_authors.jsonl | jq -r 'select(.author_ids | length >0).doc_id' | sort | uniq  > ${VALID_SET_FN}      

# Note: we can assume from the stage four that the doc_ids.txt are unique already
